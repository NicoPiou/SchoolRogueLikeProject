...
public class MediumController : MonoBehaviour {
	...
	void GroundChecking()
	{
		...
		for (int i = 0; i < 7; i++) 
		{
			...
			int layerMask = ~(LayerMask.GetMask("MediumPlayer"));
			RaycastHit2D hit = Physics2D.Raycast (rayStartPoint, Vector2.down, 0.6f, layerMask);
			if ( hit.collider != null && hit.collider.tag != "MediumPlayer") 
			{
				onGround = true;
				break;
			}
	...
	private void Moving ()
	{
		...
		if (onGround) 
		{
			straffe = Input.GetAxisRaw ("Horizontal2") * (((speed * fadeSpeed)*groundSpeed));
		...
		{
			straffe = Input.GetAxis ("Horizontal2") * speed;
			straffe *= Time.deltaTime;
	...
	void Jumping ()
	{
		if (Input.GetButtonDown ("Jump2") && onGround) 
		{
